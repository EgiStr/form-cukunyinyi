      ...
      <nav className="bg-white shadow-md py-1 md:px-8 flex justify-between items-center fixed top-0 left-0 right-0 z-50">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-gray-700 font-semibold">
            Ekowisata Cuku Nyi Nyi
          </span>
        </div>

        <div className="hidden md:flex items-center space-x-6 text-gray-600">
          <Link to="/scan" className="hover:text-gray-800">
            Kenali Mangrove
          </Link>
          ...
          {isLoggedIn ? (
            <div className="flex gap-2 items-center">
              <Link to="/admin" className="bg-green-700 py-1 px-2 rounded text-white">Admin</Link>
              <button
                onClick={() => {
                  handleLogout();
                }}
          ...
          <div className="md:hidden absolute top-14 left-0 right-0 bg-white shadow-lg flex flex-col items-center space-y-4 py-4 text-gray-600 z-50">
            ...
            {isLoggedIn ? (
              <div className="flex flex-col space-y-3 items-center">
                <Link to="/admin" className="bg-green-500 text-white py-1 px-2 rounded hover:bg-green-600">Admin</Link>
                <button
                  onClick={() => {
                    handleLogout();
                    setIsMenuOpen(false);
