...
import { MODEL_PATH } from "@/constant/apiPath";
const KnowMangrove = () => {
  ...
  const captureImage = () => {
    ...
    setImageFile(base64ToFile(imageSrc, "image.png", "image/png"));
  };
  ...
  const uploadFile = (binary) => {
    ...
    setImageFile(base64ToFile(binary, "image.png", "image/png"));
  };

  return (
    <div>
      <div className="w-full flex md:w-fit fixed md:left-1/2 md:transform md:-translate-x-1/2 mx-2 top-2">
        <button className="px-3 bg-white" onClick={() => navigate("/")}>
          <svg
      ...
      {capturedImage ? (
        <>
          <div className="flex items-center justify-center w-full h-screen">
            <img
              src={capturedImage}
              className="max-w-full h-auto border-2 border-green-400"
            />
          </div>
